    ...
    for file in uploaded_files:
        with st.spinner(f'{file.name} 분석 중...'):
            try:
                ...
                if not text:
                    st.warning(f"⚠️ {file.name}에서 읽을 수 있는 텍스트가 없습니다.")
                    continue

                # AI 분석 요청
                model = genai.GenerativeModel('gemini-1.5-flash')
                prompt = f"""
                당신은 보험 전문 분석가입니다. 다음 텍스트에서 {', '.join(selected_items)} 정보를 찾아주세요.
                ...
                st.error(f"❌ {file.name} 분석 중 상세 오류: {e}")
